$.ajax({
  ...
  success: async function (data) {
    ...
    $(".navigationEp ul.subMenu:not('.open_at_load')").hide();
    $(".navigationEp li.toggleSubMenu span").each( function () {
        var TexteSpan = $(this).text();
        $(this).replaceWith('<a href="" title="Afficher le sous-menu">' + TexteSpan + '<\/a>') ;
    } ) ;
    $(".navigationEp li.toggleSubMenu > a").click( function () {
        if ($(this).next("ul.subMenu:visible").length != 0) {
            $(this).next("ul.subMenu").slideUp("normal", function () { $(this).parent().removeClass("open") } );
        }
        else {
            $(".navigationEp ul.subMenu").slideUp("normal", function () { $(this).parent().removeClass("open") });
