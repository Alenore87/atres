...
function ShowChapter(el) {
  ...
  const folder2 = `${wLink}${wTitle}/${chaptnbre}`;
...
if (!isShowNextChapterRunning) {
  ...
  if (ChapterNow === "179" || ChapterNow === "179.5") {
    ChapterNow = parseFloat(ChapterNow) + 1; // Convertit en nombre et ajoute 0.5 --------- EDIT: SLRAGNAROK = RETRAIT DE L'OPTION !
  } else {
  ...
  const folder2 = `${wLink}${wTitle}/${ChapterNew}`;
